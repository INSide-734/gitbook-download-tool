...
import { downloadGitbook } from '../core/downloader';
...
async function handleDownload(url, options) {
	...
	try {
		...
		spinner.succeed(green('文档下载完成！'));
		console.log(blue(`文档已保存至: ${options.output}`));
	} catch (error) {
		spinner.fail(red('下载失败'));
		console.error(red(`错误信息: ${error.message}`));
		process.exit(1);
	}
}
