...
import { URL } from 'url';
import { createTurndownService, extractPageContent } from './html-parser';
import { escapeRegExp, ensureDirectory, writeFile } from '../utils';
...
async function downloadGitbook(url, options) {
	...
	try {
		...
		if (hasPath && !all) {
			...
			// 生成文件名
			const fileName = basename(urlObj.pathname) || 'index';
			await writeFile(join(outputDir, `${fileName}.md`), markdown);
		} else {
			// 如果只有域名，下载整站
			// 解析目录结构
...
async function createIndexFile(outputDir, title, tocStructure) {
	...
	// 写入索引文件
	await writeFile(join(outputDir, 'README.md'), indexContent);
}
...
async function processPages(browser, tocStructure, options) {
	...
	for (let i = 0; i < tocStructure.length; i++) {
		...
		const fileDir = join(outputDir, dirname(relativePath));
		let fileName;
		...
		// 写入文件
		await writeFile(join(fileDir, fileName), markdown);
...
async function processImages(page, markdown, imagesDir, pageUrl) {
	...
	const pageImagesDir = join(imagesDir, dirname(relativePagePath));
	...
	while ((match = imgRegex.exec(markdown)) !== null) {
		...
		const imgPath = join(pageImagesDir, imgFileName);
		const relativeImgPath = join('images', dirname(relativePagePath), imgFileName)
			.replace(/\\/g, '/');
			...
			page
				...
				.then(async (base64Data) => {
					if (base64Data) {
						...
						if (base64Content) {
							await _writeFile(imgPath, Buffer.from(base64Content, 'base64'));
